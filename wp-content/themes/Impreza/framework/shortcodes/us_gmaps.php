...
foreach ( $atts['markers'] as $index => $marker ) {
	...
	if ( ! empty( $marker['marker_address'] ) ) {
		$script_options['markers'][] = array(
			...
			'marker_size' => ( ! empty( $marker['marker_size'] ) ) ? array( $marker['marker_size'], $marker['marker_size'] ) : NULL,
		);
	}
}
...
// Output the element
$output = '<div class="w-map' . $classes . '" id="us_map_' . $us_gmaps_index . '"' . $inline_css . '>';

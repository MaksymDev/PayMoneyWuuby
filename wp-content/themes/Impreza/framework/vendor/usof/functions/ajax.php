...
function usof_ajax_color_palette() {
	...
	$theme_name = $theme->get( 'Name' );
	$paletteColors = get_option( 'usof_color_palette_' . $theme_name );
	...
	if ( isset( $color['value'] ) AND $paletteLength < 8 ) {
		...
		update_option( 'usof_color_palette_' . $theme_name, $paletteColors, TRUE );
		...
		update_option( 'usof_color_palette_' . $theme_name, $newPalette, TRUE );
		foreach ( $paletteColors as $color ) {
			$output .= '<div class="colpick_palette_value"><span style="background:' . $color . '" title="' . esc_attr( $color ) . '"></span><div class="colpick_palette_delete" title="' . us_translate( 'Delete' ) . '"></div></div>';
		}
		$output .= '<div class="colpick_palette_add" title="' . __( 'Add the current color to the palette', 'us' ) . '"></div>';
